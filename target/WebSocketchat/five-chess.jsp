	...
	function success(a,b,c,d,temp,count)//判断是否为5颗
{
	...
	{
	    winner="黑棋胜利";
	    var winperson = blackPlayer;
	    var loseperson = whitePlayer;
	}else {
        winner="白棋胜利";
        var winperson = whitePlayer;
        var loseperson = blackPlayer;
	...
	// 胜利者向数据库中传输游戏记录
	if(userid == winperson){
        $.post("${pageContext.request.contextPath}/game/insertrecord",{winperson:winperson,loseperson:loseperson},
            function (res) {
            if (res ==true){
                layer.msg("比赛记录完成")
            }else {
